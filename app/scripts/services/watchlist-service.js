...
angular.module('stockDogApp')
  .service('WatchlistService', function WatchlistService() {
    ...
    // Augment watchlist with additional helper functions
    var WatchlistModel ={
      addStock: function (stock) {
        var existingStock = _.find(this.stock, function (s) {
          return s.company.symbol === stock.company.symbol;
        });
        if (existingStock) {
          existingStock.shared += stock.shares;
        } else {
      ...
      recalculate: function() {
        var calcs = _.reduce(this.stocks, function (calcs, stock) {
          ...
          calcs.marketValue += stock.dayChange;
          return calcs;
    ...
    var findById = function (listId) {
      ...
      // [7] Initialise Model for this singleton service
      var Model = loadModel();
  });
